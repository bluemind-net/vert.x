...
public class DefaultHttpClient implements HttpClient {
	...
	private final HttpPool pool = new PriorityHttpConnectionPool() {
		protected void connect(Handler<ClientConnection> connectHandler, Handler<Throwable> connectErrorHandler,
	...
	@Override
	public HttpClient connectWebsocket(final String uri, final WebSocketVersion wsVersion,
	...
	@Override
	public HttpClient connectWebsocket(final String uri, final WebSocketVersion wsVersion, final MultiMap headers,
			final Handler<WebSocket> wsConnect) {
	...
	@Override
	public HttpClient connectWebsocket(final String uri, final WebSocketVersion wsVersion, final MultiMap headers,
			final Set<String> subprotocols, final Handler<WebSocket> wsConnect) {
		checkClosed();
		configurable = false;
		...
		return new Handler<HttpClientResponse>() {
			...
			public void handle(final HttpClientResponse event) {
				...
				if (event.statusCode() == 200) {
					...
					// beside this also pause the socket for now so the user has a chance to
					// register its dataHandler
	...
	void getConnection(Handler<ClientConnection> handler, Handler<Throwable> connectionExceptionHandler,
			DefaultContext context) {
	...
	void internalConnect(final Handler<ClientConnection> connectHandler, final Handler<Throwable> connectErrorHandler) {
		if (bootstrap == null) {
			...
			bootstrap.channel(ChannelClasses.clientSocket());
	...
	private void createConn(Channel ch, Handler<ClientConnection> connectHandler) {
		final ClientConnection conn = new ClientConnection(vertx, DefaultHttpClient.this, ch, tcpHelper.isSSL(), host,
				port, keepAlive, pipelining, actualCtx);
		conn.closeHandler(new VoidHandler() {
			public void handle() {
				// The connection has been closed - tell the pool about it, this allows the pool
				// to create more
				// connections. Note the pool doesn't actually remove the connection, when the
				// next person to get a connection
	...
	private void connectionFailed(final Channel ch, final Handler<Throwable> connectionExceptionHandler,
			final Throwable t) {
		// If no specific exception handler is provided, fall back to the HttpClient's
		// exception handler.
		final Handler<Throwable> exHandler = connectionExceptionHandler == null ? exceptionHandler
		...
		protected void doMessageReceived(ClientConnection conn, ChannelHandlerContext ctx, Object msg) {
			...
			if (msg instanceof HttpContent) {
				...
				case TEXT:
					...
					break;
				case PING:
					// Echo back the content of the PING frame as PONG frame as specified in RFC
					// 6455 Section 5.5.2
	...
	protected void finalize() throws Throwable {
		...
		// so as not to leave connections and resources dangling until the system is
		// shutdown
