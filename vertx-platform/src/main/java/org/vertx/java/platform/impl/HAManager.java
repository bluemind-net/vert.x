 ...
 * HA deployments waiting for a quorum.
 *
 * If false, we check if there are any HA deployments current deployed, and if
 * so undeploy them, and add them to the list of deployments waiting for a
 * quorum.
 *
...
public class HAManager {
	...
	public HAManager(VertxInternal vertx, ServerID serverID, PlatformManagerInternal platformManager,
		...
		haInfo.putObject("server_id",
				new JsonObject().putString("host", serverID.host).putNumber("port", serverID.port));
	...
	// when an HA module is undeployed
	...
	private void callFailoverCompleteHandler(FailoverCompleteHandler handler, String nodeID, JsonObject haInfo,
			boolean result) {
		if (handler != null) {
			try {
	...
	// synchronize this in case the cluster manager is naughty and calls it
	// concurrently
	private synchronized void nodeAdded(final String nodeID) {
		// This is not ideal but we need to wait for the group information to appear -
		// and this will be shortly
	...
	private synchronized void nodeLeft(String leftNodeID) {
		...
		if (attainedQuorum) {
			...
			// We also check for and potentially resume any previous failovers that might
			// have failed
			// We can determine this if there any ids in the cluster map which aren't in the
			// node list
					...
					vertx.executeBlocking(new Action<Void>() {
						...
						public Void perform() {
							if (System.currentTimeMillis() - start > 10000) {
								...
								try {
									// Remove any context we have here (from the timer) otherwise will screw things
									// up when modules are deployed
	...
	private void checkQuorum() {
		if (quorumSize == 0) {
			...
			if (!attainedQuorum && attained) {
				// A quorum has been attained so we can deploy any currently undeployed HA
				// deployments
				...
				log.info(
						"There is no longer a quorum. Any HA deployments will be undeployed until a quorum is re-attained");
				this.attainedQuorum = false;
			}
		}
	}

	// Add some information on a deployment in the cluster so other nodes know about
	// it
	...
	// Add the deployment to an internal list of deployments - these will be
	// executed when a quorum is attained
	private void addToHADeployList(final String moduleName, final JsonObject config, final int instances,
		...
		toDeployOnQuorum.add(new Runnable() {
			public void run() {
				...
				try {
					vertx.setContext(null);
					log.info("BlueMind nothing will be deployed");
				} finally {
					...
					platformManager.undeploy(entry.getKey(), new AsyncResultHandler<Void>() {
						...
						public void handle(AsyncResult<Void> result) {
							if (result.succeeded()) {
								...
								log.info("Successfully undeployed HA deployment " + dep.modID
										+ " as there is no quorum");
								addToHADeployList(dep.modID.toString(), dep.config, dep.instances,
										new AsyncResultHandler<String>() {
											@Override
											public void handle(AsyncResult<String> result) {
												if (result.succeeded()) {
													log.info("Successfully redeployed module " + entry.getValue().modID
															+ " after quorum was re-attained");
												} else {
													log.error("Failed to redeploy module " + entry.getValue().modID
															+ " after quorum was re-attained", result.cause());
												}
											}
										});
							} else {
		...
		try {
			...
			if (chosen != null && chosen.equals(this.nodeID)) {
				if (deployments != null && deployments.size() > 0) {
					log.info("Node " + failedNodeID + " has failed. This node will deploy " + deployments.size()
							+ " deployments from that node.");
	...
	private void processFailover(JsonObject failedModule) {
		throw new VertxException("Removed in BlueMind");
	}
